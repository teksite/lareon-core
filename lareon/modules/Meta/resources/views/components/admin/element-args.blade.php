    ...
    <div>
        ...
        <div class="dynamicGroup_{{$randomItem}}" id="{{$rand}}-{{$loop->index}}" x-data="{ removeField() { this.$el.parentElement.remove(); }}">
            <div class=" mb-3 flex justify-between items-center gap-6">
                <div class="w-full">
                    <x-lareon::inputs.label :title="__('arg') . ' #'. ($loop->iteration)" for="dynamic_field-{{$rand}}-{{$loop->index}}"/>
        ...
        <div x-data="function handler(){return { fields: [], addNewField(){this.fields.push({ txt1: '', txt2: ''});},removeField(index){ this.fields.splice(index, 1);}}}">
            <div>
                    ...
                    <div class="dynamicGroup" x-bind:id="`dynamicGroup_${index + lngth + 1}`">
                        <div class="my-3 flex justify-between items-center gap-6">
                            <div class="w-full">
                                ...
                                <label x-text:="`{{__('arg')}} #${index + lngth + 1}`" x-bind:for="`dynamic_new_item-${index + lngth + 1}`" class="block font-medium text-xs text-gray-600  mb-2">{{__('new :title',['title'=>__('arg')])}}</label>
                ...
                <div class="my-3">
                    <button type="button" role="button" title="{{__('add argument')}}" id="addDynamic_{{$randomItem}}"
                                            @click="addNewField()">
                        {{__('add')}}
                    </button>
