    ...
    <fieldset class="fieldset">
        ...
        <div class="space-y-6">
            @foreach($value ?? [] as $key=>$item)
                ...
                <div class="dynamicGroup_{{$randomItem}} bordering rounded-lg p-3" id="content-content-{{$rand}}-{{$loop->index}}" x-data="{ removeField() { document.getElementById('content-content-{{$rand}}-{{$loop->index}}').remove(); }}">
                    <div class="mb-3 flex justify-between items-center gap-6">
                        <div class="w-full">
                            ...
                            <x-lareon::inputs.text name="{{$name}}[data][{{$loop->index}}][question]" id="dynamic_item_title-{{$rand}}-{{$loop->index}}" class="block w-full" :value="$item['question'] ?? ''" :required="true"/>
                        </div>
                    ...
                    <div class="w-full">
                        ...
                        <x-lareon::inputs.textarea name="{{$name}}[data][{{$loop->index}}][answer]" id="dynamic_item_answer-{{$rand}}-{{$loop->index}}" class="block w-full" :required="true">{{$item['answer'] ?? ''}}</x-lareon::inputs.textarea>
                    </div>
                        ...
                        <div class="dynamicGroup border border-zinc-300 p-3 mb-3" x-bind:id="`dynamicGroup_${index + lngth + 1}`">
                            <div class="my-3 flex justify-between items-center gap-6">
                                <div class="grid gap-3 md:grid-cols-2 w-full">
                                    <div class="w-full">
                                        ...
                                        <x-lareon::inputs.text x-bind:id="`dynamic_new_item_question-${index + lngth + 1}`" class="block w-full" x-model="field.txt1" x-bind:name="`{{$name}}[data][${index + lngth + 1}][question]`" :required="true"/>
                                    </div>
                            ...
                            <div>
                                <div class="w-full">
                                    ...
                                    <x-lareon::inputs.textarea x-bind:id="`dynamic_new_item_answer-${index + lngth + 1}`" class="block w-full" x-model="field.txt3" x-bind:name="`{{$name}}[data][${index + lngth + 1}][answer]`" :required="true"></x-lareon::inputs.textarea>
                                </div>
                            </div>
                        </div>
                    </template>
                    <div class="my-3">
                        <x-lareon::buttons.simple variant="outline" type="button" role="button" title="{{__('add title')}}" id="addDynamic_{{$randomItem}}" @click="addNewField()">
                            {{__('add')}}
